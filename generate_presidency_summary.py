...
from pathlib import Path  # Dynamic cross-platform path management
import smtplib
...
# --- FIXED: Dynamic Cross-Platform Path Configurations ---
    ...
    for event in events:
        ...
        if match:
            ...
            if start_str and 'T' in start_str:
                try:
                    time_part = start_str.split('T')[:5]  # Extracts exactly "11:15"
                    dt_obj = datetime.strptime(time_part, "%H:%M")
                    # FIXED: Dynamic hour zero-padding removal formatting for Windows vs Linux
                    hour_format = "%#I" if os.name == 'nt' else "%-I"
                    time_display = dt_obj.strftime(f"{hour_format}:%M%p").lower()
                except Exception:
                    pass
...
def generate_presidency_text():
    ...
    # FIXED: Platform-aware calendar day string formatting wrapper for Windows (#) vs Linux (-)
    day_format = "%#d" if os.name == 'nt' else "%-d"
    
    sun1_str = sunday1.strftime(f"%b {day_format}")
    log_print(f"This Sunday, {sun1_str}:")
    if list_sun1:
    ...
    sun2_str = sunday2.strftime(f"%b {day_format}")
    log_print(f"Next Sunday, {sun2_str}:")
    if list_sun2:
